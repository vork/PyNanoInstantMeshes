...
def _main():
    ...
    parser.add_argument(
        ...
        type=int,
        help="Initialize Intel TBB with P max number of threads per process",
        metavar="P",
    )
    parser.add_argument(
        "-b",
        ...
        if args.allocator and not os.environ.get("_TBB_MALLOC_PRELOAD"):
            ...
            if args.allocator_huge_pages:
                ...
                try:
                    ...
                    if pages == 0:
                        print(
                            "TBB: Pre-allocated huge pages are not currently reserved in the system. To reserve, run e.g.:\n"
                            "\tsudo sh -c 'echo 2000 > /proc/sys/vm/nr_hugepages'"
                        )
                    os.environ["TBB_MALLOC_USE_HUGE_PAGES"] = "1"
                except:
                    print(
                        "TBB: Failed to read number of pages from /proc/sys/vm/nr_hugepages\n"
