...
std::tuple<nb::ndarray<nb::numpy, const Float, nb::ndim<2>>,
           nb::ndarray<nb::numpy, const uint32_t, nb::ndim<2>>>
remesh(nb::ndarray<Float, nb::ndim<2>, nb::device::cpu> verts,
       nb::ndarray<uint32_t, nb::ndim<2>, nb::device::cpu> faces,
       int vertex_count, int rosy = 4, int posy = 4, Float scale = -1.0,
       int face_count = -1, Float creaseAngle = 0.0,
  ...
  if (pointcloud) {
    ...
    bvh->build();
    adj = generate_adjacency_matrix_pointcloud(V, N, bvh, stats, knn_points,
                                               deterministic);
    A.resize(V.cols());
    A.setConstant(1.0f);
  }
  ...
  if (!pointcloud) {
    ...
    if (stats.mMaximumEdgeLength * 2 > scale ||
      ...
      cout << "Input mesh is too coarse for the desired output edge length "
           ...
           << stats.mMaximumEdgeLength << "), subdividing .." << endl;
  ...
  // compute_position_singularities(mRes, sing, pos_sing, extrinsic, rosy,
  // posy); cout << "Position field has " << pos_sing.size() << "
  // singularities." << endl; timer.reset();
  ...
  MatrixXu F_extr;
  extract_faces(adj_extr, O_extr, N_extr, Nf_extr, F_extr, posy, mRes.scale(),
                crease_out, true, posy == 4, bvh, smooth_iter);
  cout << "Extraction is done. (total time: " << timeString(timer.reset())
       << ")" << endl;
  ...
  // Define shape arrays
  size_t vert_shape[2] = {num_verts, 3};
  size_t face_shape[2] = {num_faces, face_dim};

  // Dynamically allocate the Eigen matrices to ensure they live long enough
  auto o_extr_ptr = std::make_shared<MatrixXf>(O_extr);
  auto f_extr_ptr = std::make_shared<MatrixXu>(F_extr);

  // Create a capsule that holds the shared_ptr and manages its lifetime
  nb::capsule o_extr_capsule(
      new std::shared_ptr<MatrixXf>(o_extr_ptr), [](void *ptr) noexcept {
        delete static_cast<std::shared_ptr<MatrixXf> *>(ptr);
      });

  nb::capsule f_extr_capsule(
      new std::shared_ptr<MatrixXu>(f_extr_ptr), [](void *ptr) noexcept {
        delete static_cast<std::shared_ptr<MatrixXu> *>(ptr);
      });

  // Create the ndarray for vertices and faces, passing ownership with the
  // capsule
  nb::ndarray<nb::numpy, const Float, nb::ndim<2>> vert_array(
      o_extr_ptr->data(), 2, vert_shape, o_extr_capsule);

  nb::ndarray<nb::numpy, const uint32_t, nb::ndim<2>> face_array(
      f_extr_ptr->data(), 2, face_shape, f_extr_capsule);

  // Return the numpy arrays (which now safely own their data through the
  // capsule)
  return std::make_tuple(vert_array, face_array);
}

NB_MODULE(_pynim, m) {
  m.def("remesh", &remesh, "verts"_a, "faces"_a, "vertex_count"_a, "rosy"_a = 4,
        "posy"_a = 4, "scale"_a = -1.0, "face_count"_a = -1,
        "creaseAngle"_a = 0.0, "align_to_boundaries"_a = false,
        "extrinsic"_a = true, "smooth_iter"_a = 0, "knn_points"_a = 1000,
        "deterministic"_a = false,
        "Remeshes the input mesh and returns the new mesh as a tuple of "
        "vertices and faces.");
}
